...
pub fn print(root: Rc<RefCell<PidNode>>, show_pid: bool, sort: bool) {
    dfs(root, Vec::new(), false, show_pid, sort);
}
...
fn dfs(
    ...
    for (i, child) in root.borrow().children.iter().enumerate() {
        ...
        dfs(child.clone(), prefix.clone(), !last_child, show_pid, sort);
    }
}
...
pub fn add_parent(root: Rc<RefCell<PidNode>>) -> Rc<RefCell<PidNode>> {
    unsafe {
        ...
        loop {
            ...
            mapname.insert(entry.th32ProcessID, str_from_u8(&entry.szExeFile));
